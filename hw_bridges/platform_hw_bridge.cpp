...
class PlatformHardwareBridge : public rclcpp::Node {
 public:
  PlatformHardwareBridge() : Node("platform_hw_bridge") {
    auto jetson_map = getDeviceIDMap("jetson");
    auto platform_front_map = getDeviceIDMap("platform_front");
    auto platform_back_map = getDeviceIDMap("platform_back");

    // Create subscriptions for all angle topics
    for (const auto& [msg_name, id_mapping] : jetson_map) {
      if (msg_name.find("platform") == std::string::npos) {
        continue;  // Skip non-platform messages
      }
      auto sub = this->create_subscription<std_msgs::msg::Float32>(
          id_mapping.topic, 10,
          [this, can_id = id_mapping.can_id](
              const std_msgs::msg::Float32::SharedPtr msg) {
            this->angle_callback(msg, can_id);
          });
      subscriptions_.push_back(sub);
    }

    // Create publishers for all CAN IDs in platform maps
    for (const auto& [msg_name, id_mapping] : platform_front_map) {
      auto pub =
          this->create_publisher<std_msgs::msg::Float32>(id_mapping.topic, 10);
      canid_publishers_[id_mapping.can_id] = pub;
    }

    for (const auto& [msg_name, id_mapping] : platform_back_map) {
      auto pub =
          this->create_publisher<std_msgs::msg::Float32>(id_mapping.topic, 10);
      canid_publishers_[id_mapping.can_id] = pub;
    }
 ...
 private:
  ...
  // Unified angle callback that publishes angle data to CAN
  void angle_callback(const std_msgs::msg::Float32::SharedPtr angle,
                      int can_id) {
