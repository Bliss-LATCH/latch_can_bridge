...
class DriveHardwareBridge : public rclcpp::Node {
 public:
  DriveHardwareBridge() : Node("drive_hw_bridge") {
    auto jetson_map = getDeviceIDMap("jetson");
    auto drive_front_map = getDeviceIDMap("drive_front");

    // Create subscriptions for front drive topics (lin_vel, ang_vel)
    for (const auto& [msg_name, id_mapping] : jetson_map) {
      if (msg_name.find("drive_front") == std::string::npos) {
        continue;  // Skip non-drive-front messages (including back/elevation/platform)
      }
      auto sub = this->create_subscription<std_msgs::msg::Float32>(
          id_mapping.topic, 10,
          [this, can_id = id_mapping.can_id](
              const std_msgs::msg::Float32::SharedPtr msg) {
            this->velocity_callback(msg, can_id);
          });
      subscriptions_.push_back(sub);
    }

    // Create publishers for all CAN IDs in the drive_front hardware map
    for (const auto& [msg_name, id_mapping] : drive_front_map) {
      auto pub =
 ...
 private:
  ...
  // Unified velocity callback that publishes the desired velocity of the system to CAN
  void velocity_callback(const std_msgs::msg::Float32::SharedPtr vel,
                       int can_id) {
    latch_interfaces::msg::CanFrame frame;
    frame.can_id = can_id;
    frame.data_len = 4;
    memcpy(&frame.data, &vel->data, sizeof(vel->data));
